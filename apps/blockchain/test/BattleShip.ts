...
describe('Battleship', function () {
  ...
  async function getShipAddress(txResponse: ContractTransaction) {
    let txReceipt = await txResponse.wait();
    // @ts-ignore
    let [mintEvent] = txReceipt.events;
    console.log(mintEvent.args);
    return mintEvent.args[1];
  }
  ...
  describe('Create Ship', function () {
    it('Should create ship with right price', async function () {
      ...
      await expect(battleShip.connect(otherAccount).createShip({value: ethers.utils.parseEther('0.01')})).not.to.be.reverted;
    });

    it('Should not create ship with wrong price', async function () {
      const {armor, ammo, battleShip, owner, otherAccount} = await loadFixture(deployTokenFixture);
      await expect(battleShip.connect(otherAccount).createShip({value: ethers.utils.parseEther('0.001')})).to.be.revertedWith('Ship price is ETH 0.01: not enough!');
    });

    it('Should mint ammo and armor token for each new ship', async function () {
      const {armor, ammo, battleShip, owner, otherAccount} = await loadFixture(deployTokenFixture);
      await battleShip.connect(otherAccount).createShip({value: ethers.utils.parseEther('0.01')});
      expect(await ammo.balanceOf('0x0000000000000000000000000000000000000001')).to.equal(BigNumber.from('10'));
      expect(await armor.balanceOf('0x0000000000000000000000000000000000000001')).to.equal(BigNumber.from('10'));
      await battleShip.connect(otherAccount).createShip({value: ethers.utils.parseEther('0.01')});
      expect(await ammo.balanceOf('0x0000000000000000000000000000000000000002')).to.equal(BigNumber.from('10'));
      expect(await armor.balanceOf('0x0000000000000000000000000000000000000002')).to.equal(BigNumber.from('10'));
    });
  });
  ...
  describe('Attack Ship', function () {
    it('Should allow owner of a ship to attack another existing enemy ship', async function () {
      const {armor, ammo, battleShip, owner, otherAccount} = await loadFixture(deployTokenFixture);
      let txResponse = await battleShip.createShip({value: ethers.utils.parseEther('0.01')});
      let txReceipt = await txResponse.wait();
      let [mintEvent] = txReceipt.events;
      const heroShipId = mintEvent.args;
      txResponse = await battleShip.connect(otherAccount).createShip({value: ethers.utils.parseEther('0.01')});
    ...
    it('Should decrease ammo of attacker', async function() {
      ...
      let txResponse = await battleShip.createShip({value: ethers.utils.parseEther('0.01')});
      const heroShipId = await getShipId(txResponse);

      txResponse = await battleShip.connect(otherAccount).createShip({value: ethers.utils.parseEther('0.01')});
      const villainShipId = await getShipId(txResponse);
      ...
      expect(await ammo.balanceOf('0x0000000000000000000000000000000000000001')).to.equal('9');
      expect(await armor.balanceOf('0x0000000000000000000000000000000000000002')).to.equal('9');
    });
  });
