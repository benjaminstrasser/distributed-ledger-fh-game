...
describe('Battleship', function () {
  ...
  describe('Create Ship', function () {
    it('Should create ship with right price', async function () {
      const { armor, ammo, battleShip, owner, otherAccount } = await loadFixture(
        deployTokenFixture
      );
      await expect(
        battleShip
          .connect(otherAccount)
          .createShip(otherAccount.address, 'FirstShip', { value: ethers.utils.parseEther('0.01') })
      ).not.to.be.reverted;
    });
    ...
    it('Should not create ship with wrong price', async function () {
      ...
      await expect(
        battleShip.connect(otherAccount).createShip(otherAccount.address, 'FirstShip', {
          value: ethers.utils.parseEther('0.001')
        })
      ).to.be.revertedWith('Ship price is ETH 0.01: not enough!');
    });
    ...
    it('Should mint ammo and armor token for each new ship', async function () {
      ...
      await battleShip
        .connect(otherAccount)
        .createShip(otherAccount.address, 'SecondShip', { value: ethers.utils.parseEther('0.01') });
  ...
  describe('Attack Ship', function () {
    it('Should allow owner of a ship to attack another existing enemy ship', async function () {
      ...
      let txResponse = await battleShip.createShip(owner.address, 'HeroShip', {
        value: ethers.utils.parseEther('0.01')
      });
      let txReceipt = await txResponse.wait();
      let [mintEvent] = txReceipt.events;
      const heroShipId = mintEvent.args;
      txResponse = await battleShip
        .connect(otherAccount)
        .createShip(otherAccount.address, 'VillainShip', {
          value: ethers.utils.parseEther('0.01')
        });
      txReceipt = await txResponse.wait();
      [mintEvent] = txReceipt.events;
      const villainShipId = mintEvent.args;
      expect(battleShip.connect(otherAccount).attack(heroShipId, villainShipId)).to.be.revertedWith(
